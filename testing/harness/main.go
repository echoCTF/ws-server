...
const bearerServerToken = "Bearer server-token-1"
...
func runCore() []result {
	return []result{
		scenarioMissingToken(),
		scenarioInvalidToken(),
		scenarioValidTokenConnects(),
		scenarioExpiredRowStillValidates(),
		scenarioClientDataFrameRejected(),
		scenarioOversizedCookieRejected(),
		scenarioTooManyConnections(),
		scenarioPublishDeliversToOnlinePlayer(),
		scenarioPublishQueuesThenFlushesOffline(),
		scenarioPublishMissingAuth401(),
		scenarioPublishBadServerToken403(),
		scenarioPublishBadBody400(),
		scenarioBroadcastAllReachesEveryConnection(),
		scenarioBroadcastOfflinePlayerNoQueue(),
		scenarioMetricsEndpoint(),
	}
}

// scenarioMissingToken: expect close code 1008 "missing token", header
// X-WS-Reject: missing_token, no X-Player-Uid (nothing was resolved), and no
// "token" field is asserted here since we can't see server logs from the
// client, that's diffed separately from the captured log files.
func scenarioMissingToken() result {
	...
	return result{"ws_missing_token", pass, detail}
}

// scenarioInvalidToken: expect close code 1008 "invalid token", no
// X-Player-Uid (the token never resolved to a player).
func scenarioInvalidToken() result {
	...
	return result{"ws_invalid_token", pass, detail}
}

// scenarioValidTokenConnects: expect a clean upgrade, no immediate close,
// and X-Player-Uid set to the token's resolved player_id (1, for
// player-token-1 per the fixture).
func scenarioValidTokenConnects() result {
	...
	return result{"ws_valid_token_connects", pass, detail}
}

// scenarioExpiredRowStillValidates: expires_at is in the past but the row
// still exists, expect it still validates (validateToken never checks
// expires_at, see main.go).
func scenarioExpiredRowStillValidates() result {
	...
	return result{"ws_expired_row_still_validates", pass, detail}
}

// scenarioClientDataFrameRejected: client sends a data frame, expect the
// server closes with 1008 "client messages not accepted".
func scenarioClientDataFrameRejected() result {
	...
	return result{"ws_client_data_frame_rejected", pass, detail}
}

// scenarioOversizedCookieRejected: rejected with 431 before any upgrade at
// all, no X-WS-Reject header involved since this never gets that far. Uses
// a value far bigger than any sane -max-header-value default so this
// doesn't depend on what the server was actually configured with.
func scenarioOversizedCookieRejected() result {
	...
	return result{"ws_oversized_cookie_rejected_431", pass, detail}
}

// scenarioTooManyConnections: max-conns defaults to 10 on both builds. Open
// 10 for player-token-2 (exclusive to this scenario), then the 11th must be
// rejected with 1013 "too many connections".
func scenarioTooManyConnections() result {
	...
	return result{"ws_too_many_connections", ok, detail}
}

// scenarioPublishDeliversToOnlinePlayer: connect player-token-3, publish to
// player_id=3, expect the live connection receives it.
func scenarioPublishDeliversToOnlinePlayer() result {
	...
	if pass {
		...
		resp, body := httpPost("/publish",
			map[string]string{headerAuthorization: bearerServerToken, headerContentType: mimeJSON},
	...
	return result{"publish_delivers_to_online_player", pass, detail}
}

// scenarioPublishQueuesThenFlushesOffline: publish to an offline player
...
func scenarioPublishQueuesThenFlushesOffline() result {
	...
	return result{"publish_queues_then_flushes_offline", pass, detail}
}

// scenarioPublishMissingAuth401: publish with no Authorization header: 401.
func scenarioPublishMissingAuth401() result {
	resp, _ := httpPost("/publish", nil, `{"player_id":"1","event":"x","payload":{}}`)
	pass := resp != nil && resp.StatusCode == 401
	return result{"publish_missing_auth_401", pass, fmt.Sprintf("status=%v", statusOrNil(resp))}
}

// scenarioPublishBadServerToken403: publish with an invalid server token: 403.
func scenarioPublishBadServerToken403() result {
	resp, _ := httpPost("/publish",
		map[string]string{headerAuthorization: "Bearer not-a-real-token"},
		`{"player_id":"1","event":"x","payload":{}}`)
	pass := resp != nil && resp.StatusCode == 403
	return result{"publish_bad_server_token_403", pass, fmt.Sprintf("status=%v", statusOrNil(resp))}
}
...
// scenarioBroadcastAllReachesEveryConnection: broadcast with no player_id
// delivers to every connected client.
func scenarioBroadcastAllReachesEveryConnection() result {
	...
	return result{"broadcast_all_reaches_every_connection", pass, detail}
}

// scenarioBroadcastOfflinePlayerNoQueue: broadcast to a specific,
// never-connected player: 200 OK, message simply discarded (nothing to
// receive, nothing to assert beyond the status code, this is the "no
...
// scenarioMetricsEndpoint: /metrics is reachable and exposes the documented
// series names.
func scenarioMetricsEndpoint() result {
	...
	return result{"metrics_endpoint_exposes_expected_series", pass, fmt.Sprintf("err=%v len(body)=%d", err, len(body))}
}
